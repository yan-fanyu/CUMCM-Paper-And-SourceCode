...
def f2(x,u1,u2,w1,w2):
    return (6250 * math.cos(1.4005 * x) - 656.3616 * u2 -V(u1) * 1025 * 9.8 + 80000 * (w1 - u1) + 10000 * math.pow(abs(w2 - u2) , 0.5) * (w2 - u2))*(1/(4866+1335.535))

def f3(x,u1,u2,w1,w2):
    return w2

def f4(x,u1,u2,w1,w2):
    return (1/2433) * (-1*80000 * (w1 - u1) - 10000 * math.pow(abs(w2 - u2) , 0.5) * (w2 - u2))
...
if __name__ == "__main__":
    ...
    #将结果输出
    with open("D:\Desktop\\T1.2.csv", "w", newline="") as csvfile:
