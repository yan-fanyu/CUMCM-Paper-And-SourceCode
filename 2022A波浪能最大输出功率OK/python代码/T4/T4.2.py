...
if __name__ == "__main__":
    ...
    list1 = range(0,100001,5000)
    list2 = range(0,100001,5000)
    ...
    for i in range(10000):
        ...
        q1[0] = 0
        q2[0] = 0
        [tempc, tempd] = newans(c1, d1,1100, 1100)
    ...
    #
    #         RK4(u1,u2,w1,w2,p1,p2,q1,q2,x)
    #         p = [c * (u2[i] - w2[i]) * (u2[i] - w2[i]) + d * (p2[i] - q2[i]) * (p2[i] - q2[i])for i in range(15000,25000)]
    #         aver.append([c,d,sum(p) / len(p)])

    with open("D:\Desktop\\T4.csv", "w", newline="") as csvfile:
