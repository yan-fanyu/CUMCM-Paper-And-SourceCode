...
for i = 1:length(dl_index)
    for j = 1:length(update_length) 
       new_axis(j,1) = update_length(j,dl_index(i))*cosd(angle_1_mat(j,1))*cosd(angle_1_mat(j,2));
       new_axis(j,2) = update_length(j,dl_index(i))*cosd(angle_1_mat(j,1))*sind(angle_1_mat(j,2));
       new_axis(j,3) = update_length(j,dl_index(i))*sind(angle_1_mat(j,1));
    end
    [new_angle, new_line, temp_a, temp_b, temp_c, index_3(i,1)] = triangle_calculator(fit_triangle);
end
% global r_mat;
