...
%   根据满足条件的各点横坐标计算各点到c的距离，将多次的结果都存储在update_length中, print_up是各下拉锁移动距离
    ...
    %         检索当前角度的主索节点
            ...
            for j = 1:length(angle_x)
                update_length(angle_x(j,1),int32(num/2+dl/pace)) = up_result(i)/cos(unique_rad(i));
            end
...
%     假如促动器全部满足条件，视为成功,将dl存储,并且画出横坐标与伸长量print_up的图,并且记录最大移动量和最小移动量
    if fail_up == 0
...
%       假如促动器全部满足条件，视为成功,将dl存储,并且画出横坐标与伸长量print_down的图,并且记录最大移动量和最小移动量
    if fail_down == 0
        success_dl(success_index,1) = -dl;
        success_index = success_index + 1;
